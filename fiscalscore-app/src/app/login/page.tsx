    ...
    <div className="min-h-screen bg-slate-50 px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto w-full max-w-md rounded-3xl border border-gray-200 bg-white p-10 shadow-sm">
        <div className="mb-8 text-center">
          <p className="text-sm font-semibold uppercase tracking-[0.3em] text-sky-500">FiscalScore</p>
          <h1 className="mt-4 text-2xl font-semibold text-gray-900">Connexion</h1>
          <p className="mt-2 text-sm text-gray-500">Connectez-vous pour accéder à votre dashboard fiscal.</p>
        </div>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input
              ...
              placeholder="admin@fiscalscore.sn"
            />
          </div>
          <div>
          ...
          {(feedback || error) && (
            <div className="rounded-2xl bg-red-50 p-3 text-sm text-red-700">{feedback || "Erreur de connexion"}</div>
          )}
          <button
            type="submit"
            disabled={loading}

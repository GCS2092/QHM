...
export default async function EditQuestionnairePage({ params }: EditQuestionnairePageProps) {
  ...
  if (!questionnaire) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Modifier le questionnaire</h1>
          <p className="text-sm text-gray-500 mt-1">Questionnaire introuvable ou inaccessible.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Modifier le questionnaire</h1>
        <p className="text-sm text-gray-500 mt-1">Mettez à jour le titre, la description et l’état du questionnaire.</p>
      </div>
      <QuestionnaireEditForm questionnaire={questionnaire} />
    </div>
  );
}
